...
class ConvVAETrainer(object):
    def __init__(
        ...
        print("in vae trainer, matching loss coef: ", matching_loss_coef)
    ...
    def matching_loss(self, batch):
        latents_ori = self.model.encode(batch)[0]

        masked_batch = batch.detach().clone()
        _, imlen = masked_batch.shape
        mask = (np.random.uniform(size=imlen) > 0.5).astype(np.float)
        masked_batch = masked_batch * ptu.from_numpy(mask) # mask all images for training vae latent space

        latents_masked = self.model.encode(masked_batch)[0]
        
        loss = F.mse_loss(latents_ori, latents_masked)
        return loss  

    def train_epoch(self, epoch, sample_batch=None, batches=25, from_rl=False, key=None):
        self.model.train()
        losses = []
        log_probs = []
        kles = []
        ...
        for batch_idx in range(batches):
            if sample_batch is not None:
                data = sample_batch(self.batch_size, epoch, key=key)
                # obs = data['obs']
            ...
            loss = -1 * log_prob + beta * kle + self.matching_loss_coef
    ...
    def test_epoch(
        ...
        for batch_idx in range(10):
            ...
            if self.matching_loss_coef > 0:
                ...
                matching_loss = ptu.zeros(1)
            loss = -1 * log_prob + beta * kle + self.matching_loss_coef * matching_loss
