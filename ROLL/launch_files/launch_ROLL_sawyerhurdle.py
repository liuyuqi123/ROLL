...
def main(mode, debug, dry):
    ...
    variant=dict(
            ...
            skewfit_variant=dict(
                ...
                segmentation_kwargs=dict(
                    segment=True, # if false, object-vae will just be the same as scene-vae
                    dilation=False,
                    show=False,
                    robot_threshold=0.05,
                    fg_threshold=120,
                ),
                keep_train_segmentation_lstm=True, # whehter to train segmentation lstm online.
                save_video=True,
                ...
                online_lstm_trainer_kwargs=dict(
                    beta=0, # stop training vae part online
                    recon_loss_coef=0, # stop training vae part online
                    triplet_loss_coef=[],
                    triplet_loss_type=[],
                    triplet_loss_margin=1,
                    matching_loss_coef=50,
                    vae_matching_loss_coef=0, # stop training vae part online
                    ae_loss_coef=0.5,
                    lstm_kl_loss_coef=0,
            ...
            train_vae_variant=dict( # actually this is train vae and lstm variant
                only_train_lstm=False, # if terminate the process just after training the ROLL.
                lstm_version=2,
                ...
                lstm_pretrain_vae_only=False, # if true, will only use random sampled images (not trajs) to train the first vae part, no training of the ROLL.
                ...
                generate_lstm_dataset_kwargs=dict( 
                    N=500, # pretrain lstm dataset size
                    test_p=.9,
                    show=False,
                    occlusion_prob=0.3,
                    occlusion_level=0.5,
                ...
                # this is the kwargs for pretraining ROLL.
                algo_kwargs=dict(
                ...
                save_period=100, # no need to store so many figures
            ),
        )
    ...
    exp_prefix = '1030-ROLL-release-push-hurdle-top'
    print("exp prefix is: ", exp_prefix)

    # handle lstm architecture
    if variant['train_vae_variant']['lstm_version'] == 3 or variant['train_vae_variant']['lstm_version'] == 2:
        lstm_48_default_architecture['LSTM_args']['input_size'] = variant['train_vae_variant']['representation_size']
        # lstm_48_default_architecture['conv_args']['output_size'] = 576
    ...
    if variant['skewfit_variant']['keep_train_segmentation_lstm']:
        ...
        variant['train_vae_variant']['generate_lstm_dataset_kwargs']['N'] = 2000
    ...
    if debug: # use very small parameters to make sure code at least compiles and can run
        ...
        for vv in vg.variants():
            vv['variant']['skewfit_variant']['algo_kwargs']['batch_size'] = 3
            vv['variant']['skewfit_variant']['online_vae_trainer_kwargs']['batch_size'] = 3
            vv['variant']['skewfit_variant']['online_lstm_trainer_kwargs']['batch_size'] = 3
        ...
        #     import copy
        #     variants = []
        #     for vae_matching_loss_coef in [0, 1000]:
        #         variant_ = copy.deepcopy(variant)
        #         variant_['train_vae_variant']['algo_kwargs']['vae_matching_loss_coef'] = vae_matching_loss_coef
        #         variants.append(variant_)
        #     vg.add('variant', variants)
        # else:
        vg.add('variant', [variant])
